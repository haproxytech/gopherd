...
// chowned the file to root (i.e. when the host UID is non-root). -u 0 forces
// root even when the container runs --user (rootless mode), which exec inherits.
func (dc *detachedContainer) writeConfig(content string) {
	dc.t.Helper()
	cmd := exec.Command("docker", "exec", "-u", "0", "-i", dc.id, "tee", "/test/gopherd.yml")
